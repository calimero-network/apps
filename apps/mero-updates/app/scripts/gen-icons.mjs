// Generates Mero Updates' icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — three rising bars, a KPI chart — is kept in sync by hand with
// public/favicon.svg, the only asset here that is NOT generated.
// Adapted from mero-stream's generator; rasteriser and encoders unchanged.
...
// ---- art -------------------------------------------------------------------
// Three rising bars — the KPI chart every update carries. Blue, so it is not
// mistaken at 16 px for mero-forum (amber), mero-pass (green) or mero-stream
// (violet), which sit beside it in the launcher.
...
function mark(c, fit) {
  const paint = grad(14, 48, 50, 14, FROM, TO);
  // Bars share a baseline at y=50 and climb left to right; wide enough (9u)
  // to stay three distinct bars at 16 px.
  draw(
    c,
    union(
      rrect(14, 36, 9, 14, 3),
      rrect(27.5, 27, 9, 23, 3),
      rrect(41, 14, 9, 36, 3),
    ),
    paint,
    fit,
