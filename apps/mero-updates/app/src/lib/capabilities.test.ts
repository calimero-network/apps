...
describe("space capabilities", () => {
  ...
  it("lets a member do every non-governing thing the app asks of them", () => {
    ...
    // audience worked for the namespace owner (who holds full caps regardless of
  ...
  it("withholds authorship-on-behalf from both roles", () => {
    // Lets a node publish writes attributed to ANOTHER member. In an audience, that
    // is posting under someone else's name. No call site here needs it.
