 ...
 *
 * Absent on codes minted before audiences existed — treat that as "namespace".
 */
export type InviteKind = "namespace" | "audience";
...
export interface AudienceInvitePayload {
  invitation: SignedInvitation;
  /** Namespace name. curb calls this groupAlias; kept for cross-app compatibility. */
  groupAlias?: string;
  /** Optional: jump straight into this audience's context after joining. */
  contextId?: string;
  /** The group `invitation` grants — normally the namespace. */
  groupId?: string;
  /**
   * What this code is FOR: land the joiner in the namespace, or in one audience of it.
   * Absent on pre-audiences codes ⇒ "namespace".
   *
   * Note this describes the destination, not the grant. On current nodes an audience
   * code still grants the NAMESPACE (see `mintAudienceInvite`), because audience access is
   * inherited from it.
   */
  kind?: InviteKind;
  /**
   * The audience (subgroup) to open after joining. A ROUTING HINT and nothing more —
   * it is outside the signature, so it cannot grant anything. Entering the audience
   * still requires the node to admit us, which it only does for a member of the
   * parent namespace.
   */
  audienceId?: string;
  /** Display name of the audience, when this is an audience invite. */
  audienceName?: string;
  /**
...
function parseChain(raw: unknown): InviteChainEntry[] | undefined {
  ...
  for (const e of raw) {
    ...
    out.push({
      ...
      // leave the joiner outside the audience they were invited to.
      kind: entry.kind === "namespace" ? "namespace" : "audience",
    });
  }
  return out.length > 0 ? out : undefined;
}

function parsePayload(json: string): AudienceInvitePayload | null {
  try {
    ...
    if (isSignedInvitation((inner as AudienceInvitePayload).invitation)) {
      const p = inner as AudienceInvitePayload & { groupName?: string };
      return {
        ...
        kind:
          p.kind === "audience"
            ...
            : p.kind === "namespace"
              ? "namespace"
              : undefined,
        audienceId: typeof p.audienceId === "string" ? p.audienceId : undefined,
        audienceName: typeof p.audienceName === "string" ? p.audienceName : undefined,
...
export function encodeInvite(payload: AudienceInvitePayload): string {
 ...
 */
export function decodeInvite(input: string): AudienceInvitePayload | null {
 ...
 * namespace, for an audience invite it is the audience's subgroup. Same field either way,
...
export function groupIdOfInvite(
  invitation: SignedInvitation | AudienceInvitePayload,
): string {
  const signed = (
    "invitation" in invitation &&
    isSignedInvitation((invitation as AudienceInvitePayload).invitation)
      ? (invitation as AudienceInvitePayload).invitation
