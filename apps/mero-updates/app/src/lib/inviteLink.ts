...
//
//   https://links.calimero.network/com.calimero.mero-updates/join?invitation=<code>
//
...
//
//   1. **It pointed wherever the Invite button happened to be.** An audience invite
//      minted on `/spaces/:namespaceId` produced a link to that path, and the
//      redemption effect lived on `/spaces` — so the recipient landed on a
 ...
 */
export const APP_SLUG = "com.calimero.mero-updates";
 ...
 * slug like `com.calimero.mero-updates`.
 *
...
export function invitationFromRaw(raw: string): string | null {
  ...
  // this app's own older link (`/spaces?invite=…`) comes back as
  // `{slug: "spaces", action: null}` — a route misread as a slug, which it has
