...
NODE_NAME="mero-design-dev-2"
NODE_HOME="${MERODESIGN_DEV_NODE2_HOME:-$HOME/.calimero/mero-design-dev-2}"
NODE_PORT="${MERODESIGN_DEV_PORT2:-2431}"
NODE_P2P_PORT="${MERODESIGN_DEV_P2P_PORT2:-2531}"
...
WASM_PATH="$REPO_ROOT/logic/res/mero_design.wasm"
...
pid_file() { echo "/tmp/mero-design-dev-node2.pid"; }
...
[ -f "$WASM_PATH" ] || { red "mero-design.wasm not found — run 'make dev-node' first"; exit 1; }
...
NODE1_LOG="/tmp/mero-design-dev-node.log"
NODE1_PEER_ID=""
if [ -f "$NODE1_LOG" ]; then
...
merod --node "$NODE_NAME" --home "$NODE_HOME" run \
  --auth-mode embedded \
  > "/tmp/mero-design-dev-node2.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node2 started (pid $!  logs: /tmp/mero-design-dev-node2.log)"
wait_for_node
...
step "Installing MeroDesign app on node2"
...
printf '  Logs:       /tmp/mero-design-dev-node2.log\n'
printf '\n'
printf '  Next step:\n'
