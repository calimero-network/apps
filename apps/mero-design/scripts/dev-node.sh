...
NODE_NAME="mero-design-dev"
NODE_HOME="${MERODESIGN_DEV_NODE_HOME:-$HOME/.calimero/mero-design-dev}"
...
pid_file() { echo "/tmp/mero-design-dev-node.pid"; }
...
merod --node "$NODE_NAME" --home "$NODE_HOME" run \
  --auth-mode embedded \
  > "/tmp/mero-design-dev-node.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node started (pid $!  logs: /tmp/mero-design-dev-node.log)"
wait_for_node
...
printf '  Logs:       /tmp/mero-design-dev-node.log\n'
printf '\n'
printf '  Next step:\n'
