// Aliased over ../../src/lib/groups by vite.config.ts, so the teams and rooms
...
const ROOMS: RoomRow[] = [
  ...
  {
    // A room whose context has not replicated to this node yet cannot be
    // entered at all — the state the UI used to render as an inert button.
...
export async function listTeamNamespaces(): Promise<NamespaceRow[]> {
