/**
 * Landing page contract for Mero Meet.
 *
...
test.describe('Mero Meet landing page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
  });

  test('hero names the app and its tagline', async ({ page }) => {
    await expect(page.getByRole('heading', { level: 1, name: "Mero Meet" })).toBeVisible();
  ...
  test('shows the availability badge', async ({ page }) => {
    ...
    await expect(
      page.locator('.cal-lp-badge').filter({ hasText: "Desktop only" }),
    ).toHaveCount(1);
  });
