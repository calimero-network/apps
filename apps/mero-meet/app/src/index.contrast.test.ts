...
describe("call-surface inks", () => {
  // The call stage is dark in BOTH themes, so text on it is theme-invariant and
  // is checked against the stage rather than the page background. Getting this
  // wrong is how mero-stream ended up painting a light-mode ink (#404040) onto
  // a near-black surface at 1.5:1.
  it.each([
    [':root,\n:root[data-theme="light"] {'],
    [':root[data-theme="dark"] {'],
  ])("call inks clear AA on --call-bg and --tile-bg in %s", (ROOT) => {
    for (const ink of ["on-media", "media-dim"]) {
      for (const surface of ["call-bg", "tile-bg"]) {
        expect(
          contrast(token(ROOT, ink), token(ROOT, surface)),
        ).toBeGreaterThanOrEqual(AA);
      }
    }
  });
