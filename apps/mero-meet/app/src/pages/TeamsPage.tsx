...
/**
 * Teams = NAMESPACES. One level up from where this page used to sit.
 *
 * It used to list contexts and call each one a "team", which collapsed two
 * distinct things into one and left no room for rooms: creating a team made a
 ...
 *
 *   Namespace ("team")  ← you invite people HERE
 *     └── Subgroup ("room") + Context   ← one video call   → /teams/:id
 *
...
export default function TeamsPage() {
  ...
  const load = useCallback(
    async (showSpinner = true) => {
      ...
      try {
        setNamespaces(await listTeamNamespaces(mero.admin, appId));
      } catch (e) {
  ...
  const create = useCallback(() => {
    ...
    void run("create", async (onStatus) => {
      const { namespaceId } = await createTeamNamespace(
        mero.admin,
        { applicationId: appId, name: teamName },
        onStatus,
      );
      setName("");
      onStatus("Refreshing your teams…");
      await load(false);
    ...
    (raw: string) => {
      ...
      void run("join", async (onStatus) => {
        ...
        if (landed.kind === "namespace") {
          navigate(`/teams/${landed.namespaceId}`);
          return;
        }
        showToast("Joined. Your teams are listed below.");
      });
    },
    ...
    <div className={styles.page}>
      ...
      <main className={styles.content}>
        <div className={styles.heading}>
          ...
          <p className={styles.subtitle}>
            A <strong>team</strong> is a namespace you invite people to. Inside
            it, each <strong>room</strong> is one meeting — its own lobby, chat
            and call. Invite people to the team once and every room in it is
            open to them.
          </p>
        </div>
        ...
        {status && (
          <StatusNote tone="pending" testId="teams-status">
            {status}
          </StatusNote>
        )}
        {error && (
          <StatusNote tone="error" testId="teams-error">
            {error}
          </StatusNote>
        )}
        ...
        {/* A modal, not an inline panel. Expanding in place pushed the list
            ...
            for one team reading as current while you looked at another. */}
        <InviteModal
          open={!!invite}
          code={invite?.code ?? ""}
          scope={`Whole team · ${
            namespaces.find((n) => n.namespaceId === invite?.id)?.name ?? "team"
          }`}
          onClose={() => setInvite(null)}
          hint={
            <>
              Anyone with this link can join that team and every room in it.
              Opening it shows them what they have been invited to and a Join
              button — in the web app, the installed app, or the desktop
              launcher. To invite someone into one specific call, open the team
              and use <strong>Invite</strong> on that room.
            </>
          }
        />
        ...
        <div className={styles.sectionHead}>
          <h3 className={styles.sectionTitle}>
            {namespaces.length} team{namespaces.length === 1 ? "" : "s"}
          </h3>
        ...
        {/* "Not installed" is its own state, distinct from "no teams". The
            list is scoped to this app's id, so without one there is nothing to
            scope BY — showing "No teams yet" there invites you to create one,
        ...
        {notInstalled && (
          <div className={styles.empty}>
            ...
            <span className={styles.emptyHint}>
              Install it from the marketplace, then reload. Teams are listed per
              application, so there is nothing to show until the node has this
              one.
            </span>
          </div>
        )}
        ...
        {!listing &&
          ...
          namespaces.length === 0 && (
            <div className={styles.empty}>
              <span className={styles.emptyTitle}>No teams yet</span>
              <span className={styles.emptyHint}>
              ...
              >
                ...
                <div className={styles.cardActions}>
                  <button
                    ...
                    onClick={() => navigate(`/teams/${ns.namespaceId}`)}
      ...
      >
        <div className={styles.dialogHead}>
          <h2 className={styles.dialogTitle}>Join a team or room</h2>
          <span className={styles.spacer} />
          <button
            type="button"
            className={styles.ghostBtn}
