...
//
//   Namespace  = the team / workspace        ← invite people here
...
export async function listTeamNamespaces(
  admin: AdminLike,
  ...
  return (namespaces ?? []).map((n) => ({
    ...
    name: (n.name ?? "").trim() || `Team ${n.namespaceId.slice(0, 6)}`,
...
/**
 * Create the namespace that holds a team's rooms.
 *
 ...
 */
export async function createTeamNamespace(
  admin: AdminLike,
...
export type Redeemed =
  | {
      ...
      roomName?: string;
      /**
       * The team this room belongs to, when the invitation named it. Carried
 ...
 * inline sequence in TeamsPage, which meant the app-wide invitation prompt
 ...
 * second leaves someone a member of a team staring at a call they cannot enter.
 */
...
/**
 * Get into a room's call context, and return the member identity to team as.
 *
 ...
 * one is "wait or rejoin the team", the other is "this room can never admit
 * anyone invited to the team". Guessing sends people to check a setting that
...
async function diagnoseAdmission(
  ...
  if (visibility === "restricted") {
    return (
      "The room is RESTRICTED, so being in the team does not admit you — " +
  ...
  if (!namespaceId) {
    return (
      "This node cannot see which team the room belongs to, which means the " +
  ...
  if (visibility === "open") {
    return (
      "The room is open, so this is your membership of the team not having " +
  ...
  return (
    ...
    "team has not reached this node yet, or the room was created restricted."
  );
}
