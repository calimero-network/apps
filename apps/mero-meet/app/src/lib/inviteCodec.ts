...
function parsePayload(json: string): TeamInvitePayload | null {
  try {
    ...
    if (isSignedInvitation((inner as TeamInvitePayload).invitation)) {
      const p = inner as TeamInvitePayload & { groupName?: string };
      return {
...
export function encodeInvite(payload: TeamInvitePayload): string {
 ...
 */
export function decodeInvite(input: string): TeamInvitePayload | null {
...
export function groupIdOfInvite(
  invitation: SignedInvitation | TeamInvitePayload,
): string {
  const signed = (
    "invitation" in invitation &&
    isSignedInvitation((invitation as TeamInvitePayload).invitation)
      ? (invitation as TeamInvitePayload).invitation
