...
//
//   https://links.calimero.network/com.calimero.mero-meet/join?invitation=<code>
//
...
//      minted on `/teams/:namespaceId` produced a link to that path, and the
//      redemption effect lived on `/teams` — so the recipient landed on a
 ...
 * slug like `com.calimero.mero-meet`.
 *
...
export function invitationFromRaw(raw: string): string | null {
  ...
  // this app's own older link (`/teams?invite=…`) comes back as
  // `{slug: "teams", action: null}` — a route misread as a slug, which it has
