...
describe("APP_SLUG", () => {
  it("is the package id from the contract manifest, not a friendly name", () => {
    ...
    expect(APP_SLUG).toBe("com.calimero.mero-meet");
  });
});
...
describe("invitationUrl", () => {
  ...
  it("does not depend on where the app is currently mounted", () => {
    ...
    // invitation minted on /teams/:id used to produce a link to that path,
...
describe("invitationDeepLink", () => {
  ...
  it("round-trips, and a DOTTED slug survives parsing", () => {
    ...
    // dotted host on a non-special scheme, so `com.calimero.mero-meet` would
...
describe("invitationFromRaw", () => {
  ...
  it("still reads this app's OLDER ?invite= links", () => {
    ...
    expect(
      invitationFromRaw(
        `https://mero.example/teams?${LEGACY_INVITE_PARAM}=${CODE}`,
      ),
    ).toBe(CODE);
  });
  ...
  it("does not mistake a PATH SEGMENT for another app's slug", () => {
    ...
    // so this app's own legacy `/teams?invite=…` arrives as
    // `{slug: "teams", action: null}`. Rejecting on the slug alone threw away
    // exactly the links the legacy branch exists to keep working, so the guard
    // requires a full slug+action platform shape.
    expect(
      invitationFromRaw(
        `https://mero.example/teams?${INVITATION_PARAM}=${CODE}`,
      ),
    ).toBe(CODE);
    expect(
      invitationFromRaw(
  ...
  it("returns null when there is no invitation in it", () => {
    ...
    expect(invitationFromRaw("   ")).toBeNull();
    expect(invitationFromRaw("https://mero.example/teams")).toBeNull();
...
describe("urlWithoutInvitation", () => {
  ...
  it("is a no-op on a URL with no query", () => {
    expect(urlWithoutInvitation("https://x.dev/teams#tok")).toBe(
      "https://x.dev/teams#tok",
    );
  });
});
