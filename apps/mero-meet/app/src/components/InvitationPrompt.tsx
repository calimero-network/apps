...
import { decodeInvite, type TeamInvitePayload } from "../lib/inviteCodec";
import {
  onInvitation,
 ...
 *
 * Because a link can land anywhere. The first version put this on the teams
 * page, which is wrong in a way that had already bitten once: someone with a
 * stored context is redirected to `/lobby` on boot, so the teams page never
...
export default function InvitationPrompt() {
  ...
  const name =
    pending.payload.roomName ?? pending.payload.groupAlias ?? "a team";
          ...
          <span className={styles.sub}>
            {status ??
              ...
              (pending.payload.roomName
                ...
                : "Joining gives you access to the team and every room in it.")}
          </span>
        </span>
        <button
          type="button"
