// Generates MeroSign's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — a signature stroke over a document edge — is kept in sync by hand
...
/** A stroke of width `w` between two points, with round caps. */
const seg = (x0, y0, x1, y1, w) => {
  ...
  return (u, v) => {
    ...
    const px = x0 + t * dx;
    const py = y0 + t * dy;
    return (u - px) ** 2 + (v - py) ** 2 <= (w / 2) ** 2;
  };
};

function mark(c, fit) {
  const paint = grad(12, 14, 52, 46, FROM, TO);
  // Three strokes approximating a hand-drawn flourish: up, loop back, out.
  draw(
    c,
    union(
      seg(13, 40, 24, 20, 5),
  ...
  // The signing line it rests on.
  draw(c, rrect(12, 47, 40, 4, 2), solid(TO, 0.85), fit);
}
