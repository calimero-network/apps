 ...
 * `@calimero-network/calimero-client` — the SDK this app is still on — reads
 * only `application_id`:
 *
 ...
 *
 * It seeds the two values the SDK reads from `localStorage` at boot, under the
 * exact keys it reads them from:
 *
...
/** The key `calimero-client` reads the application id from. */
const APP_ID_KEY = 'calimero-application-id';
/** The key it reads the node url from — JSON-encoded, not a bare string. */
const NODE_URL_KEY = 'app-url';
...
export function bootstrapDesktopSession(): void {
  ...
  try {
    ...
    if (nodeUrl && !localStorage.getItem(NODE_URL_KEY)) {
      // JSON, because the SDK does `JSON.parse(localStorage.getItem('app-url'))`
      // and a bare URL throws there — which reads as "no node configured".
      localStorage.setItem(NODE_URL_KEY, JSON.stringify(nodeUrl));
    }
  } catch {
