// Generates Battleships' icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — a targeting reticle over a hit marker — is kept in sync by hand
...
function mark(c, fit) {
  const paint = grad(14, 14, 50, 50, FROM, TO);
  draw(
    c,
    union(
      // reticle
      ring(32, 32, 17, 4.5),
      // cross hairs, broken at the ring so the centre reads clearly
      rrect(30, 6, 4, 12, 2),
      rrect(30, 46, 4, 12, 2),
      rrect(6, 30, 12, 4, 2),
      rrect(46, 30, 12, 4, 2),
  ...
  // The hit, painted separately and SOLID: on the shared gradient the centre
  // landed mid-ramp and read as muddy grey rather than as a strike.
  draw(c, circle(32, 32, 6), solid(TO), fit);
}
