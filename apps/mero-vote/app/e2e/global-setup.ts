/**
 * Playwright global setup: one native merod with embedded auth, the mero-vote
 ...
 * contract-level convergence story stays in logic/workflows/, which is what
 * actually needs two nodes.
 */
...
export default async function globalSetup() {
  ...
  const context = await api<{ contextId: string }>(
    ...
    // the wrong type. mero-vote's `init` takes nothing, so it is empty.
