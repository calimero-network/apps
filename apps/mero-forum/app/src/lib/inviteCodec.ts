 ...
 *
 * Absent on codes minted before forums existed — treat that as "namespace".
 */
export type InviteKind = "namespace" | "forum";
...
export interface ForumInvitePayload {
  invitation: SignedInvitation;
  /** Namespace name. curb calls this groupAlias; kept for cross-app compatibility. */
  groupAlias?: string;
  /** Optional: jump straight into this forum's context after joining. */
  contextId?: string;
  /** The group `invitation` grants — normally the namespace. */
  groupId?: string;
  /**
   * What this code is FOR: land the joiner in the namespace, or in one forum of it.
   * Absent on pre-forums codes ⇒ "namespace".
   *
   * Note this describes the destination, not the grant. On current nodes a forum
   * code still grants the NAMESPACE (see `mintForumInvite`), because forum access is
   * inherited from it.
   */
  kind?: InviteKind;
  /**
   * The forum (subgroup) to open after joining. A ROUTING HINT and nothing more —
   * it is outside the signature, so it cannot grant anything. Entering the forum
   * still requires the node to admit us, which it only does for a member of the
   * parent namespace.
   */
  forumId?: string;
  /** Display name of the forum, when this is a forum invite. */
  forumName?: string;
  /**
...
function parseChain(raw: unknown): InviteChainEntry[] | undefined {
  ...
  for (const e of raw) {
    ...
    out.push({
      ...
      // leave the joiner outside the forum they were invited to.
      kind: entry.kind === "namespace" ? "namespace" : "forum",
    });
  }
  return out.length > 0 ? out : undefined;
}

function parsePayload(json: string): ForumInvitePayload | null {
  try {
    ...
    if (isSignedInvitation((inner as ForumInvitePayload).invitation)) {
      const p = inner as ForumInvitePayload & { groupName?: string };
      return {
        ...
        kind:
          p.kind === "forum"
            ...
            : p.kind === "namespace"
              ? "namespace"
              : undefined,
        forumId: typeof p.forumId === "string" ? p.forumId : undefined,
        forumName: typeof p.forumName === "string" ? p.forumName : undefined,
...
export function encodeInvite(payload: ForumInvitePayload): string {
 ...
 */
export function decodeInvite(input: string): ForumInvitePayload | null {
 ...
 * namespace, for a forum invite it is the forum's subgroup. Same field either way,
...
export function groupIdOfInvite(
  invitation: SignedInvitation | ForumInvitePayload,
): string {
  const signed = (
    "invitation" in invitation &&
    isSignedInvitation((invitation as ForumInvitePayload).invitation)
