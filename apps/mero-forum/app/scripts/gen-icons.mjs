// Generates Mero Forum's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — three stacked threads — is kept in sync by hand with
...
function mark(c, fit) {
  const paint = grad(14, 16, 50, 48, FROM, TO);
  // Rows at 3 widths: a list that reads as a list even when it is 16 px tall.
  draw(
    c,
    union(
      rrect(14, 18, 36, 7, 3.5),
      rrect(14, 30, 28, 7, 3.5),
      rrect(14, 42, 20, 7, 3.5),
    ),
    paint,
    fit,
  );
