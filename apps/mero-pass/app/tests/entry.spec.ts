...
// going missing.
//
// `/` is Mero Pass's own landing page (src/pages/landing). Its contract —
// chapters, the Cloud band, the comparison, the FAQ — is asserted in
// tests/marketing-landing.spec.ts; what stays here is the way in.
test.describe('unauthenticated entry', () => {
  test('the entry route renders and offers a way to connect', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('the page identifies itself as Mero Pass', async ({ page }) => {
    ...
    await expect(page.locator('.mp-ld-header__brand')).toHaveAccessibleName(
      'Mero Pass',
    );
  });
