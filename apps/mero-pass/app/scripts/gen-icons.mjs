// Generates MeroPass's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — a padlock — is kept in sync by hand with public/favicon.svg, the
// only asset here that is NOT generated.
// Run: node scripts/gen-icons.mjs
//
// Adapted from mero-stream's generator; the rasteriser and encoders are
...
// ---- art -------------------------------------------------------------------
// A padlock, in emerald → teal. BOTH axes differ from the apps this sits next
// to in the launcher: mero-stream and mero-meet share a violet palette and a
// play/camera silhouette, so a lock in green is distinguishable at 16 px
// without reading the label — which is the whole job of a favicon.

const BG = "#090b10";
const FROM = "#34d399";
const TO = "#0d9488";

/** A ring of width `w`, unrestricted in angle — the shackle needs an arc about
 *  the −y axis and the shared `arc()` primitive only spans about +x. */
const ring = (cx, cy, r, w) => (u, v) => {
  const d = Math.hypot(u - cx, v - cy);
  return d >= r - w / 2 && d <= r + w / 2;
};

/** Everything above `y` — clips the ring into a shackle. */
const above = (f, y) => (u, v) => v <= y && f(u, v);

/** Set union, so a shape can be assembled from parts in one draw call and get
 *  ONE anti-aliased pass rather than seams where the parts abut. */
const union = (...fs) => (u, v) => fs.some((f) => f(u, v));

const SHACKLE_R = 10.5;
const SHACKLE_W = 5;
const SHACKLE_CY = 27;
const BODY = { x: 13, y: 26, w: 38, h: 28, r: 7 };

function mark(c, fit) {
  const paint = grad(13, 16, 51, 54, FROM, TO);

  // Shackle: a half-ring with round caps, the caps squared off where the body
  // covers them.
  draw(
    c,
    union(
      above(ring(32, SHACKLE_CY, SHACKLE_R, SHACKLE_W), SHACKLE_CY),
      circle(32 - SHACKLE_R, SHACKLE_CY, SHACKLE_W / 2),
      circle(32 + SHACKLE_R, SHACKLE_CY, SHACKLE_W / 2),
    ),
    paint,
    fit,
  ...
  // Body.
  draw(c, rrect(BODY.x, BODY.y, BODY.w, BODY.h, BODY.r), paint, fit);

  // Keyhole, punched back out in the background colour. A hole rather than a
  // lighter fill so it survives the OS masking that recolours nothing.
  draw(
    c,
    union(circle(32, 37, 4), poly([[30, 37], [34, 37], [33, 47], [31, 47]])),
    solid(BG),
    fit,
  );
}
