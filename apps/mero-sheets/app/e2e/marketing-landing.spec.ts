/**
 * Landing page contract for Mero Sheets — all three pages of it.
 *
...
test.describe('Mero Sheets landing page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
  });

  test('hero names the app and its tagline', async ({ page }) => {
    await expect(page.getByRole('heading', { level: 1, name: "Mero Sheets" })).toBeVisible();
  ...
  test('the overview carries its extra sections', async ({ page }) => {
    await expect(page.locator('.cal-lp-headline')).toHaveText("The spreadsheet your team actually owns.");
    ...
    await expect(page.locator('#showcase .cal-lp-chapter')).toHaveCount(4);
  ...
  test('a showcase chapter seeks the clip and becomes the current one', async ({ page }) => {
    ...
    expect(t).toBeGreaterThanOrEqual(14.5);
  });
