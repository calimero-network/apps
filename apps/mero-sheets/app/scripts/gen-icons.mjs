// Generates Mero Sheets' icon set into app/public/ with zero dependencies:
...
// ---- art -------------------------------------------------------------------
// A header bar over a 2x2 body with one cell filled. The filled cell is the
// whole idea: a bare grid reads as a generic table or a window, and the
// selected cell is what says SPREADSHEET at 16 px. Calimero green (#a4ff11,
// the accent this app already uses) on its own near-black background.

const BG = "#0e140f";

function mark(c, fit) {
  draw(c, rrect(12, 14, 38, 9, 2), grad(12, 14, 50, 23, "#a4ff11", "#93e60c"), fit);
  const cells = [[12, 26], [32, 26], [12, 38], [32, 38]];
  cells.forEach(([x, y], i) => {
    draw(c, rrect(x, y, 18, 10, 2), i === 3 ? solid("#a4ff11") : solid("#f5f8f1", 0.22), fit);
  });
}
