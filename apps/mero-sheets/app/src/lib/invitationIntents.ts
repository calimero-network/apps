...
//      app sees the intent. mero-sheets bounces an authenticated visitor from
//      `/` to `/mero-sheets`, so a link that landed on `/` would be gone before
//      any page-level effect could read it.
...
//      and login — which matters here, because an invitation link opened by
//      someone not signed in goes through the whole login redirect first.
//
...
import { JOIN_ACTION, invitationFromRaw, urlWithoutInvitation } from './inviteLink';
...
function storage(): Storage {
  try {
    ...
    const probe = '__mero_sheets_probe__';
