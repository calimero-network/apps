...
describe("stage inks", () => {
  // The stage is dark in BOTH themes, so its text tokens are theme-invariant and
  // are checked against the stage rather than against the page background. This
  // is the pairing that broke: `--text-dim` (#404040 in light) was used for the
  // "Nobody is broadcasting yet" heading sitting on the near-black stage.
  it.each([
    [':root,\n:root[data-theme="light"] {'],
    [':root[data-theme="dark"] {'],
  ])("--stage-text clears AA on --tile and --stage in %s", (ROOT) => {
    for (const surface of ["stage", "tile"]) {
      expect(
        contrast(token(ROOT, "stage-text"), token(ROOT, surface)),
      ).toBeGreaterThanOrEqual(AA);
    }
  });
});
