...
describe("createStreamNamespace — the rc.41 default mask", () => {
  it("never sends CAN_AUTHOR_ON_BEHALF", async () => {
    const admin = fakeAdmin();
    await createStreamNamespace(admin as unknown as AdminLike, {
      applicationId: "app-1",
      name: "Stream",
    });
  ...
  it("fails the whole call when the node refuses the write", async () => {
    ...
    await expect(
      createStreamNamespace(admin as unknown as AdminLike, {
        applicationId: "app-1",
        name: "Stream",
      }),
    ).rejects.toThrow("503");
