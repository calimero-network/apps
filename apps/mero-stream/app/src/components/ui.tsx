...
/**
 * A minted invite code: the code itself, a copy button that confirms, and the
 * scope it grants.
 *
 * Saying the scope out loud matters — a namespace code and a room code look
 * identical (both are one base58 blob) but grant different things, and handing
 * someone the wrong one is a confusing failure much later, at join time.
 */
export function InviteBox({
  ...
  return (
    <section className={styles.inviteBox} data-testid="invite-box">
      <div className={styles.inviteHead}>
        <h3 className={styles.inviteTitle}>Invite code</h3>
        <span className={styles.inviteScope} data-testid="invite-scope">
          {scope}
        </span>
      </div>
      ...
      {hint && <p className={styles.inviteHint}>{hint}</p>}
    </section>
  );
}
