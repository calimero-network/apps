...
#
# Builds the WASM, runs a node, installs Mero Stream, and creates a room
...
NODE_NAME="stream-dev"
NODE_HOME="${STREAM_DEV_NODE_HOME:-$HOME/.calimero/stream-dev}"
...
pid_file() { echo "/tmp/stream-dev-node.pid"; }
...
step "Starting node1"
merod --node "$NODE_NAME" --home "$NODE_HOME" run > "/tmp/stream-dev-node.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node1 started (pid $!  logs: /tmp/stream-dev-node.log)"
wait_for_node
...
step "Installing Mero Stream app"
