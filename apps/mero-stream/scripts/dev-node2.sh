...
# two merods on one host), installs Mero Stream, and appends its URL/tokens to
...
NODE_NAME="stream-dev-2"
NODE_HOME="${STREAM_DEV_NODE2_HOME:-$HOME/.calimero/stream-dev-2}"
...
NODE1_LOG="/tmp/stream-dev-node.log"

ADMIN_USER="${MEET_ADMIN_USER:-admin}"
ADMIN_PASS="${MEET_ADMIN_PASS:-calimero1234}"

WASM_PATH="$REPO_ROOT/logic/res/mero_stream.wasm"
...
pid_file() { echo "/tmp/stream-dev-node2.pid"; }
...
step "Starting node2"
merod --node "$NODE_NAME" --home "$NODE_HOME" run > "/tmp/stream-dev-node2.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node2 started (pid $!  logs: /tmp/stream-dev-node2.log)"
wait_for_node
...
step "Installing Mero Stream app on node2"
