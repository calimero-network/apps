...
NODE_NAME="mero-pixart-dev"
NODE_HOME="${MEROPIXART_DEV_NODE_HOME:-$HOME/.calimero/mero-pixart-dev}"
...
pid_file() { echo "/tmp/mero-pixart-dev-node.pid"; }
...
merod --node "$NODE_NAME" --home "$NODE_HOME" run \
  --auth-mode embedded \
  > "/tmp/mero-pixart-dev-node.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node started (pid $!  logs: /tmp/mero-pixart-dev-node.log)"
wait_for_node
...
printf '  Logs:       /tmp/mero-pixart-dev-node.log\n'
printf '\n'
printf '  Next step:\n'
