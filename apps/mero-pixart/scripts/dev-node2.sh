#!/usr/bin/env bash
# scripts/dev-node2.sh — Start a second local merod node for MeroPixArt development.
#
# Usage:
...
NODE_NAME="mero-pixart-dev-2"
NODE_HOME="${MEROPIXART_DEV_NODE2_HOME:-$HOME/.calimero/mero-pixart-dev-2}"
NODE_PORT="${MEROPIXART_DEV_PORT2:-2461}"
NODE_P2P_PORT="${MEROPIXART_DEV_P2P_PORT2:-2561}"
NODE_URL="http://localhost:${NODE_PORT}"

NODE1_P2P_PORT="${MEROPIXART_DEV_P2P_PORT:-2560}"

ADMIN_USER="${E2E_ADMIN_USER:-admin}"
ADMIN_PASS="${E2E_ADMIN_PASS:-calimero1234}"

WASM_PATH="$REPO_ROOT/logic/res/mero_pixart.wasm"
...
pid_file() { echo "/tmp/mero-pixart-dev-node2.pid"; }
...
[ -f "$WASM_PATH" ] || { red "mero-pixart.wasm not found — run 'make dev-node' first"; exit 1; }
...
NODE1_LOG="/tmp/mero-pixart-dev-node.log"
NODE1_PEER_ID=""
if [ -f "$NODE1_LOG" ]; then
...
merod --node "$NODE_NAME" --home "$NODE_HOME" run \
  --auth-mode embedded \
  > "/tmp/mero-pixart-dev-node2.log" 2>&1 &
echo $! > "$(pid_file)"
green "Node2 started (pid $!  logs: /tmp/mero-pixart-dev-node2.log)"
wait_for_node
...
step "Installing MeroPixArt app on node2"
...
printf '  Logs:       /tmp/mero-pixart-dev-node2.log\n'
printf '\n'
printf '  Next step:\n'
