...
/// Per-document record.
///
/// `#[app::mergeable]` supplies the deterministic re-key cascade: a nested
/// collection stored under a value type that is not a registered `RekeyTarget`
/// keeps a per-replica random storage id and never converges. The attribute
/// rather than `#[derive(Mergeable)]` because `created_at: u64` is immutable
/// plain data the derive cannot field-merge.
///
#[app::mergeable(id = "mero_drive::DocRecord")]
#[derive(BorshSerialize, BorshDeserialize, AbiType)]
...
impl DocsState {
    ...
    pub(crate) fn edit_doc_inner(
        ...
        // Mutate in place via a write-back guard: an in-place edit preserves the
        // record's already-deterministic entity id so no re-key is needed.
        let mut rec = self
            .docs
            .get_mut(&id)
...
mod tests {
    ...
    fn doc_record_merge_is_idempotent() {
        // `DocRecord` is not `Clone`, so build two stubs with the same logical
