//! Emit the registry service's ABI manifest alongside the built `.wasm`.
//! The client's `calimero-abi-codegen` reads `res/abi.json` to generate
//! the `RegistryClient` TypeScript class.
