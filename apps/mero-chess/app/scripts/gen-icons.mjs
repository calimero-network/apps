// Generates Mero Chess's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — a pawn standing on a checkered tile — is kept in sync by hand with
// public/favicon.svg, the only asset here that is NOT generated.
// Adapted from kv-store's generator; rasteriser and encoders unchanged, with
// `circle` and `poly` added back for a shape that is not a rectangle.
...
// ---- shapes (design units) -------------------------------------------------
// `rrect` for the tile and the squares, `circle` and `poly` for the pawn. Each
// one is a PREDICATE over design-unit coordinates — the rasteriser above
// supersamples it, so a shape needs no anti-aliasing of its own.
...
/** Even-odd point-in-polygon over [x, y] pairs. */
const poly = (points) => (u, v) => {
  let inside = false;
...
// ---- art -------------------------------------------------------------------
// A pawn standing on two checkered squares. The checker is what says "chess" at
// 16px, where the pawn is barely three pixels wide; the pawn is what says it at
// 512, where the checker is background texture. Geometry and colours are the
// same numbers as public/favicon.svg.

const BG = "#0b0e14";
const SQUARE = "#1b2130";
const PIECE = "#7dd3fc";

/** The two squares of the checker, drawn dark so the pawn stays the subject. */
const SQUARES = [
  { x: 8, y: 8, w: 24, h: 24 },
  { x: 32, y: 32, w: 24, h: 24 },
];

/** Head, collar, body and base — a pawn in four parts. */
function pawn(c, fit) {
  const ink = solid(PIECE);
  draw(c, circle(32, 21, 8), ink, fit);
  draw(c, rrect(27, 27, 10, 4, 2), ink, fit);
  draw(
    c,
    poly([
      [25, 45],
  ...
  draw(c, rrect(20, 44, 24, 7, 3), ink, fit);
}

function mark(c, fit) {
  for (const { x, y, w, h } of SQUARES) {
    draw(c, rrect(x, y, w, h, 2), solid(SQUARE), fit);
  }
  pawn(c, fit);
}
