#!/usr/bin/env bash
# Three native merod nodes, the mero-docs bundle, a mesh, and app/.env.integration.
# Every process it starts is recorded in its own pid file and stopped only from there.
set -euo pipefail

RIG_DIR="${MERODOCS_RIG_DIR:-/tmp/merodocs-rig}" # wiped by `up`; holds node homes, logs and the pid file
...
mint_token() {
  curl -sf "$(node_url "$1")/auth/token" \
    ...
    -d "$(jq -nc --arg u "$ADMIN_USER" --arg p "$ADMIN_PASSWORD" \
      ...
      '{auth_method:"user_password",public_key:$k,client_name:"mero-docs-rig",timestamp:$t,permissions:["admin"],provider_data:{username:$u,password:$p}}')" ||
...
write_workflow() {
  ...
  {
    cat <<YAML
name: mero-docs local rig
description: >
  Installs the mero-docs bundle on every rig node, creates the namespace, a
