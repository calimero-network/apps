...
for (const svc of config.services) {
  ...
  // The monorepo promotes the PRIMARY service's crate to logic/ (a member glob
  // match must be a package, so logic/Cargo.toml cannot be a nested workspace).
  // Any additional service crate stays under logic/crates/<name>/. Probe for the
  // promoted layout first and fall back, so this works either way.
  ...
  // src/generated, not src/api: ci.yml's staleness gate globs
  // `apps/*/app/src/generated`, so a client generated anywhere else is
  // regenerated by `pnpm -r codegen` and then never diffed — the gate silently
  // covers nothing.
  const out = resolve(APP_DIR, 'src', 'generated');
