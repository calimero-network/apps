// Generates Mero CRM's icon set into app/public/ with zero dependencies:
...
// ---- art -------------------------------------------------------------------
// A sales funnel: three stacked bars, each narrower than the last — the
// pipeline narrowing toward won. Plain rounded rects so it still reads at 16 px.
// Calimero green (#b6ff5e, this app's accent) on its own near-black.

const BG = "#0e140f";

function mark(c, fit) {
  const paint = grad(14, 16, 50, 48, "#b6ff5e", "#a4ff11");
  draw(c, rrect(12, 14, 40, 9, 4.5), paint, fit);
  draw(c, rrect(18, 27.5, 28, 9, 4.5), paint, fit);
  draw(c, rrect(25, 41, 14, 9, 4.5), paint, fit);
}
