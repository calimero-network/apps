...
export interface NewDealFields {
  title: string;
  /** Typed as a person would: `12000`, `12k`. */
  value?: string;
  organization?: string;
  /** Stage id to create the deal in (default: the modal's first stage). */
  stageId?: string;
}

/**
 * Open the New-deal modal, fill it, and create the deal. Only the title is
 * required; everything else has a default in the modal.
 */
export async function createDeal(page: Page, fields: NewDealFields): Promise<void> {
  await page.getByTestId('open-new-deal-btn').click();
  await page.getByTestId('deal-title-input').fill(fields.title);
  if (fields.value) await page.getByTestId('deal-value-input').fill(fields.value);
  if (fields.organization) await page.getByTestId('deal-org-input').fill(fields.organization);
  if (fields.stageId) await page.getByTestId('deal-stage-select').selectOption(fields.stageId);
  await page.getByTestId('deal-create-submit').click();
  await page.getByTestId('deal-create-submit').waitFor({ state: 'detached', timeout: 15_000 });
}
...
export async function loginViaHash(page: Page, nodeIndex = 0, opts: { inject?: boolean } = {}) {
  ...
  // explicit onboarding (empty state) instead of a pre-provisioned pipeline.
 ...
 * namespace, skip the alias gate, and add a pipeline (so the board becomes ready).
...
export async function driveOnboarding(page: Page) {
  ...
  await page.getByTestId('pipeline-add-name').fill(uniqueName('pipeline'));
  await page.getByTestId('pipeline-add-submit').click();
}

/**
 * Ensure `page` is inside a ready workspace (a pipeline is selected). Isolated runs
...
export async function createWorkspace(page: Page) {
  ...
  // still binding leaves activeNs/activePipeline split across two namespaces, so
...
export async function inviteAndJoin(inviterPage: Page, joinerPage: Page): Promise<string> {
  ...
  // Restore the inviter to the pipeline board: this helper detoured through the
  // Members view to reach the Invite button, but callers expect to be back on
  // the board (where deal cards live) once the invite is sent.
  await inviterPage.getByTestId('nav-pipeline').click().catch(() => {});
...
export async function clearAuth(page: Page) {
  try {
    ...
    await page.evaluate(() => {
      ...
      // Per-namespace pipeline selection + alias-gate markers.
      for (let i = localStorage.length - 1; i >= 0; i--) {
        const k = localStorage.key(i);
        if (k && (k.startsWith('crm:activePipeline:') || k.startsWith('crm:alias-set:'))) {
