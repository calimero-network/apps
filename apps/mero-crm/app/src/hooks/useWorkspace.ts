 ...
 *  - A context inside the namespace is ONE pipeline. Pipelines are added explicitly
 *    (name + currency); `activePipeline` (a contextId) is persisted per
 *    namespace and feeds every deal view.
 ...
 *      * pipeline name      -> the context metadata record (`setContextMetadata`),
 ...
 *    treat that context as the active pipeline and resolve its namespace, skipping
...
import { CrmClient } from '../generated/CrmClient';
...
export interface UseWorkspaceReturn {
  /**
   * A workspace joined this session whose pipelines have not replicated yet. The
   * sidebar shows "syncing" instead of "No pipelines yet", which is otherwise what
  ...
  // pipelines (contexts inside the active namespace)
  pipelines: PipelineEntry[];
  activePipeline: string | null;
  selectPipeline: (contextId: string) => void;
  addPipeline: (name: string, currency: string) => Promise<string | null>;
  addPipelineLoading: boolean;
  addPipelineError: Error | null;
...
export function useWorkspace(): UseWorkspaceReturn {
  ...
  // --- Contexts (pipelines) in the active namespace ---
  const { contexts, loading: pipelinesLoading, refetch: refetchContexts } =
    useGroupContexts(activeNs);

  // Pipeline names, from the REPLICATED context metadata record.
  //
  ...
  // — but an invited member's node never received that label, so the same pipeline
  ...
  // `recipes/context-metadata`; `addPipeline` now writes it).
  //
  ...
  // for every pipeline on every refetch.
  const contextIdsKey = useMemo(
    () => contexts.map((c) => c.contextId).join(','),
    [contexts],
  );
  const [pipelineMetaNames, setPipelineMetaNames] = useState<Map<string, string>>(new Map());
  useEffect(() => {
    const ids = contextIdsKey ? contextIdsKey.split(',') : [];
    if (!mero || !activeNs || ids.length === 0) { setPipelineMetaNames(new Map()); return; }
    let cancelled = false;
    (async () => {
      ...
      if (cancelled) return;
      setPipelineMetaNames(new Map(entries.filter((e): e is [string, string] => e !== null)));
    })();
    return () => { cancelled = true; };
  ...
  const pipelines = useMemo<PipelineEntry[]>(
    () =>
      contexts.map((c) => ({
        ...
        name:
          pipelineMetaNames.get(c.contextId) ||
          c.name?.trim() ||
          c.contextId.slice(0, 8),
      })),
    [contexts, pipelineMetaNames],
  );

  // --- Active pipeline (a contextId; persisted per namespace) ---
  const [activePipeline, setActivePipeline] = useState<string | null>(callbackContextId);
  const userSelectedPipeline = useRef(false);

  // On a namespace switch, drop the previous namespace's pipeline so a stale
  // contextId never leaks into the new namespace's views before its context
  // list resolves. The SSO path pins its own pipeline and is exempt.
  useEffect(() => {
    if (callbackContextId) return;
    userSelectedPipeline.current = false;
    setActivePipeline(null);
  }, [activeNs, callbackContextId]);

  // Prefer the SSO callback context; otherwise auto-select the persisted pipeline
  // if it still exists, else the first pipeline, else none (add-pipeline empty state).
  useEffect(() => {
    if (callbackContextId) { setActivePipeline(callbackContextId); return; }
    if (!activeNs) { setActivePipeline(null); return; }
    if (userSelectedPipeline.current && activePipeline && pipelines.some((r) => r.contextId === activePipeline)) {
      return;
    }
    const persisted = readActivePipeline(activeNs);
    if (persisted && pipelines.some((r) => r.contextId === persisted)) {
      setActivePipeline(persisted);
      return;
    }
    if (activePipeline && pipelines.some((r) => r.contextId === activePipeline)) return;
    setActivePipeline(pipelines[0]?.contextId ?? null);
  }, [callbackContextId, activeNs, pipelines, activePipeline]);

  const selectPipeline = useCallback((contextId: string) => {
    userSelectedPipeline.current = true;
    setActivePipeline(contextId);
    if (activeNs) writeActivePipeline(activeNs, contextId);
  }, [activeNs]);

  // --- Executor identity for the active pipeline context (for RPC) ---
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // every pipeline that already existed when someone accepted an invitation
  ...
  // --- Mutations: create namespace / add pipeline / join / invite ---
  ...
  const createNamespace = useCallback(
    async (name: string): Promise<string | null> => {
      ...
      try {
        ...
        // What every member of this workspace may do: add a pipeline and invite
  ...
  const [addPipelineLoading, setAddPipelineLoading] = useState(false);
  const [addPipelineError, setAddPipelineError] = useState<Error | null>(null);
  const addPipeline = useCallback(
    async (name: string, currency: string): Promise<string | null> => {
      ...
      if (!trimmedName) {
        setAddPipelineError(new Error('Pipeline name is required'));
        return null;
      }
      setAddPipelineLoading(true);
      ...
      try {
        ...
        // The pipeline starts in USD. Anything else is written into shared
        // state here; best-effort, because Settings can change it later and a
        // currency is not worth failing an otherwise-created pipeline over.
        if (code && code !== 'USD') {
          try {
            await new CrmClient(mero, ctx.contextId).setCurrency({ currency: code });
          } catch { /* stays USD until changed in Settings */ }
        }
        ...
        // `a1b2c3d4` where the creator sees the pipeline's name.
        //
        // Best-effort: a nameless pipeline still works, and a metadata write is not
        // worth failing an otherwise-created pipeline over.
        try {
        ...
        // Best-effort node alias so tools can resolve the pipeline by name.
        try {
          await mero.admin.createContextAlias({ alias: trimmedName, contextId: ctx.contextId });
        } catch { /* convenience only */ }
        await refetchContexts();
        selectPipeline(ctx.contextId);
        return ctx.contextId;
      } catch (err) {
        setAddPipelineError(err instanceof Error ? err : new Error(String(err)));
        return null;
      } finally {
        setAddPipelineLoading(false);
      }
    },
    [mero, applicationId, activeNs, refetchContexts, selectPipeline],
  );
  ...
  const join = useCallback(async (code: string) => {
    ...
    // Joined; this workspace's pipelines have not replicated yet. Flagged so the
    // sidebar says "syncing" rather than "No pipelines yet" — which is what a
  ...
  // `pipelinesLoading` going false is the signal the pipeline list answered — for the
  ...
  useEffect(() => {
    if (!activeNs || pipelinesLoading) return;
    setListedForNs(activeNs);
  }, [activeNs, pipelinesLoading]);
  ...
  return {
    ...
    setMemberName,
    refetchMembers,
    ready: activePipeline !== null && executorPublicKey !== null,
    loading: resolvingApplicationId || nsLoading || pipelinesLoading,
    error: null,
    clearPersisted,
  };
}
