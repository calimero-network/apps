...
describe('readActiveNs', () => {
  it('ignores a legacy unversioned key: an old build never auto-enters', () => {
    localStorage.setItem('crm:activeNs', 'old-namespace-id');
  ...
  it('dropLegacyActiveNs removes the old key without ever reading it', () => {
    localStorage.setItem('crm:activeNs', 'old-namespace-id');
    dropLegacyActiveNs();
    expect(localStorage.getItem('crm:activeNs')).toBeNull();
    expect(readActiveNs()).toBeNull();
  });
});

describe('activePipeline persistence', () => {
  it('is namespace-scoped and round-trips an explicit selection', () => {
    writeActivePipeline('ns-1', 'pipeline-a');
    expect(readActivePipeline('ns-1')).toBe('pipeline-a');
    expect(readActivePipeline('ns-2')).toBeNull();
  });
});

describe('clearPersistedWorkspace', () => {
  it('drops the active namespace and every per-namespace pipeline/alias key', () => {
    writeActiveNs('ns-1');
    writeActivePipeline('ns-1', 'pipeline-a');
    localStorage.setItem('crm:alias-set:ns-1', '1');
    clearPersistedWorkspace();
    expect(readActiveNs()).toBeNull();
    expect(readActivePipeline('ns-1')).toBeNull();
    expect(localStorage.getItem('crm:alias-set:ns-1')).toBeNull();
  });
});
