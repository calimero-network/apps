/**
 * Namespace/pipeline selection persistence.
 *
 ...
 */
const LEGACY_ACTIVE_NS_KEY = 'crm:activeNs';
const ACTIVE_NS_KEY = 'crm:activeNs:v2';
const activePipelineKey = (nsId: string) => `crm:activePipeline:v2:${nsId}`;
...
export function readActivePipeline(nsId: string): string | null {
  return readLs(activePipelineKey(nsId));
}
export function writeActivePipeline(nsId: string, id: string | null): void {
  writeLs(activePipelineKey(nsId), id);
}
