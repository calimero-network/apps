...
/**
 * What an ordinary member of THIS app needs to be useful: add a pipeline (a context)
...
/** May this member add a pipeline (create a context)? */
export function canCreatePipeline(input: EffectiveInput): boolean {
...
export function describeCapabilities(capabilities: number): string[] {
  const out: string[] = [];
  if (hasCapability(capabilities, CAP.CAN_CREATE_CONTEXT)) out.push('Add pipelines');
 ...
 * add a pipeline nor invite anyone, with nothing anywhere saying so. This is what
