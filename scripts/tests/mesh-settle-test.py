#!/usr/bin/env python3
"""Every step that JOINS must be preceded by a settle.
...
for f in sorted(glob.glob(str(ROOT / "apps/*/logic/**/*.yml"), recursive=True)):
    ...
    for i, step in enumerate(steps):
        if not isinstance(step, dict) or step.get("type") not in JOINING:
            continue
        checked += 1
        ...
        if not isinstance(prev, dict) or prev.get("type") != "wait":
            failures.append(
                f"{rel}: `{step['type']}` at step #{i} is not preceded by a `wait` — "
                f"it will race KeyDelivery and lose the whole scenario"
            )
        elif (prev.get("seconds") or 0) < MIN_SETTLE:
            failures.append(
                f"{rel}: the wait before `{step['type']}` at step #{i} is "
                f"{prev.get('seconds')}s, under the {MIN_SETTLE}s floor"
            )

if failures:
    print("joining steps missing a settle:\n")
    for x in failures:
        print(f"  ✗ {x}")
    sys.exit(1)

print(f"ok: all {checked} joining steps (create_mesh/join_namespace/join_group) are settled")
